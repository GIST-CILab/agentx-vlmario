...
class BaseEvaluator(GreenAgent):
    ...
    async def run_eval(self, req: EvalRequest, updater: TaskUpdater) -> None:
        ...
        await updater.update_status(TaskState.working, f"Starting A2A evaluation for {self.game_name} ({num_maps} iterations)")
        self._init_base_parts()

        results = []
        for i in range(1, num_maps + 1):
            await updater.update_status(TaskState.working, f"Requesting map {i}/{num_maps} from Designer...")
            ...
            await updater.update_status(TaskState.working, f"Map {i}: Simulation started...")
            video_path = None
            if "player" in req.participants:
                ...
                video_path = player_resp.strip() if player_resp else None
            elif self.player:
            ...
            await updater.update_status(TaskState.working, f"Map {i}: Evaluating video...")
        ...
        await updater.update_status(TaskState.working, "Evaluation complete.")
