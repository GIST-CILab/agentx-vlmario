...
from procedural_pipeline.runner import run as run_pipeline
    ...
    with Path(results_csv).open("r", encoding="utf-8-sig", newline="") as file:
        ...
        for row in reader:
            ...
            rows.append({
                ...
                "fun": to_number(row.get("enjoyment", "")),
                "challenge": to_number(row.get("difficulty", "")),
                "frustration": to_number(row.get("frustration", "")),
                "surprise": to_number(row.get("novelty", "")),
                "design": to_number(row.get("aesthetics", "")),
...
def save_outputs(output_dir: str, rows: list[dict], summary: dict) -> None:
    ...
    fieldnames = list(rows[0].keys()) if rows else [
        ...
        "fun", "challenge", "frustration", "surprise", "design", "reasoning", "error",
    ]
