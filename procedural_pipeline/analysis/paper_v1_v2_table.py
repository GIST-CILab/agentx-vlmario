...
from procedural_pipeline.analysis.truth_belief_plot import save_figure_png_and_svg

# Table 1 (paper): descriptive statistics by *perceived* AI vs Human.
# Keys match internal metric ids: fun, challenging, frustrating, surprising, design.
PAPER_PERCEIVED_AI = {
    "fun": (2.92, 1.33),
    "challenging": (3.88, 1.27),
    "frustrating": (3.60, 1.37),
    "surprising": (2.65, 1.35),
    "design": (2.70, 1.23),
}
PAPER_PERCEIVED_HUMAN = {
    "fun": (3.72, 1.20),
    "challenging": (3.65, 1.32),
    "frustrating": (2.84, 1.43),
    "surprising": (2.62, 1.26),
    "design": (3.57, 1.17),
}
...
def main() -> None:
    ...
    if args.combined:
        ...
        p_interpretation = (
            ...
            "scores in bins defined by each map’s v1 belief. Games differ in challenge profile — pooling is for "
...
def _v2_stats_by_truth(
    ...
    for mid_map in map_ids:
        ...
        for metric_id, _col in METRICS:
            val = metric_number(v2, metric_id)
            if val is not None:
...
def _save_comparison_plots(table: list[dict], path: Path, pooled: bool = False) -> None:
    ...
    save_figure_png_and_svg(fig, path, dpi=160)
    plt.close(fig)
